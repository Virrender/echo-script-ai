...
const secs = String(seconds % 60).padStart(2, "0");
    ...
    if (selected?.isNew){
        ...
        if(isCompleted){
            ...
            {transcript && (
                <div>
                <p className= {isRecording ? "text-red-500 text-xl font-bold" : "text-xl"}>
                     {minutes}:{secs}
                </p>
        ...
        return(
            <main className="flex flex-1  flex-col items-center justify-center">
                <div className='p-1'>
                        <h2 className={isRecording ? "text-yellow-500 font-bold" : ""}>
                ...
                <button className='border mt-3 rounded p-1.5 cursor-pointer '
                onClick={()=> {
                    if(isRecording){
                        stopRecording();
                    }else{
